...
/**
 * Writes tensor values and knn graphs to index segments.
 *
 * @lucene.experimental
 */
public final class Lucene99HnswTensorsWriter extends KnnVectorsWriter {

  private static final long SHALLOW_RAM_BYTES_USED =
      RamUsageEstimator.shallowSizeOfInstance(Lucene99HnswTensorsWriter.class);
  private final SegmentWriteState segmentWriteState;
  private final IndexOutput meta, tensorIndex;
  private final int M;
  private final int beamWidth;
  private final FlatTensorsWriter flatTensorWriter;
  ...
  public Lucene99HnswTensorsWriter(
      SegmentWriteState state,
      int M,
      int beamWidth,
      FlatTensorsWriter flatTensorWriter,
      int numMergeWorkers,
      TaskExecutor mergeExec)
      throws IOException {
    this.M = M;
    this.flatTensorWriter = flatTensorWriter;
    ...
    String metaFileName =
        IndexFileNames.segmentFileName(
            state.segmentInfo.name, state.segmentSuffix, Lucene99HnswTensorsFormat.META_EXTENSION);

    String indexDataFileName =
        IndexFileNames.segmentFileName(
            state.segmentInfo.name,
            state.segmentSuffix,
            Lucene99HnswTensorsFormat.TENSOR_INDEX_EXTENSION);

    boolean success = false;
    try {
      meta = state.directory.createOutput(metaFileName, state.context);
      tensorIndex = state.directory.createOutput(indexDataFileName, state.context);

      CodecUtil.writeIndexHeader(
          meta,
          Lucene99HnswTensorsFormat.META_CODEC_NAME,
          Lucene99HnswTensorsFormat.VERSION_CURRENT,
          state.segmentInfo.getId(),
          state.segmentSuffix);
      CodecUtil.writeIndexHeader(
          tensorIndex,
          Lucene99HnswTensorsFormat.TENSOR_INDEX_CODEC_NAME,
          Lucene99HnswTensorsFormat.VERSION_CURRENT,
          state.segmentInfo.getId(),
          state.segmentSuffix);
      success = true;
  ...
  public KnnFieldVectorsWriter<?> addField(FieldInfo fieldInfo) throws IOException {
    FieldWriter<?> newField =
        FieldWriter.create(
            flatTensorWriter.getFlatTensorScorer(),
            fieldInfo,
            M,
            beamWidth,
            segmentWriteState.infoStream);
    fields.add(newField);
    return flatTensorWriter.addField(fieldInfo, newField);
  }

  @Override
  public void flush(int maxDoc, Sorter.DocMap sortMap) throws IOException {
    flatTensorWriter.flush(maxDoc, sortMap);
  ...
  public void finish() throws IOException {
    ...
    if (tensorIndex != null) {
      CodecUtil.writeFooter(tensorIndex);
    }
  }
  ...
  public long ramBytesUsed() {
    ...
    total += flatTensorWriter.ramBytesUsed();
    for (FieldWriter<?> field : fields) {
  ...
  private void writeField(FieldWriter<?> fieldData) throws IOException {
    // write graph
    long tensorIndexOffset = tensorIndex.getFilePointer();
    OnHeapHnswGraph graph = fieldData.getGraph();
    int[][] graphLevelNodeOffsets = writeGraph(graph);
    long tensorIndexLength = tensorIndex.getFilePointer() - tensorIndexOffset;
  ...
  private HnswGraph reconstructAndWriteGraph(
    ...
    while (nodesOnLevel0.hasNext()) {
      ...
      long offset = tensorIndex.getFilePointer();
      reconstructAndWriteNeighbours(neighbors, oldToNewMap, maxOrd);
      levelNodeOffsets[0][node] = Math.toIntExact(tensorIndex.getFilePointer() - offset);
    }
    ...
    for (int level = 1; level < graph.numLevels(); level++) {
      ...
      for (int node : newNodes) {
        ...
        levelNodeOffsets[level][nodeOffsetIndex++] =
            Math.toIntExact(tensorIndex.getFilePointer() - offset);
      }
    }
  ...
  private void reconstructAndWriteNeighbours(NeighborArray neighbors, int[] oldToNewMap, int maxOrd)
    ...
    for (int i = 0; i < size; i++) {
      tensorIndex.writeVInt(nnodes[i]);
    }
  }
  ...
  public void mergeOneField(FieldInfo fieldInfo, MergeState mergeState) throws IOException {
    CloseableRandomVectorScorerSupplier scorerSupplier =
        flatTensorWriter.mergeOneFieldToIndex(fieldInfo, mergeState);
    boolean success = false;
    try {
      long vectorIndexOffset = tensorIndex.getFilePointer();
      ...
      if (scorerSupplier.totalVectorCount() > 0) {
        ...
        var encoding = (fieldInfo.hasTensorValues()) ?
            fieldInfo.getTensorEncoding() : fieldInfo.getVectorEncoding();
        switch (encoding) {
      ...
      long vectorIndexLength = tensorIndex.getFilePointer() - vectorIndexOffset;
      writeMeta(
          fieldInfo,
          vectorIndexOffset,
          vectorIndexLength,
  ...
  private int[][] writeGraph(OnHeapHnswGraph graph) throws IOException {
    ...
    for (int level = 0; level < graph.numLevels(); level++) {
      ...
      for (int node : sortedNodes) {
        ...
        long offsetStart = tensorIndex.getFilePointer();
        tensorIndex.writeVInt(size);
        // Destructively modify; it's ok we are discarding it after this
        int[] nnodes = neighbors.nodes();
        Arrays.sort(nnodes, 0, size);
        ...
        offsets[level][nodeOffsetId++] =
            Math.toIntExact(tensorIndex.getFilePointer() - offsetStart);
      }
    }
    return offsets;
  }
  ...
  private void writeMeta(
    ...
    if (graph == null) {
      ...
      long start = tensorIndex.getFilePointer();
      meta.writeLong(start);
      meta.writeVInt(DIRECT_MONOTONIC_BLOCK_SHIFT);
      final DirectMonotonicWriter memoryOffsetsWriter =
          DirectMonotonicWriter.getInstance(
              meta, tensorIndex, valueCount, DIRECT_MONOTONIC_BLOCK_SHIFT);
      ...
      meta.writeLong(tensorIndex.getFilePointer() - start);
    }
  }
  ...
  public void close() throws IOException {
    IOUtils.close(meta, tensorIndex, flatTensorWriter);
  }
  ...
  private static class FieldWriter<T> extends KnnFieldVectorsWriter<T> {
          ...
          switch (fieldInfo.getTensorEncoding()) {
            case BYTE -> scorer.getRandomTensorScorerSupplier(
                ...
                RandomAccessVectorValues.fromByteTensors(
                    (List<ByteTensorValue>) tensors, fieldInfo.getTensorDimension()));
            case FLOAT32 -> scorer.getRandomTensorScorerSupplier(
                fieldInfo.getTensorSimilarityFunction(),
                RandomAccessVectorValues.fromFloatTensors(
                    (List<FloatTensorValue>) tensors, fieldInfo.getTensorDimension()));
          };
      hnswGraphBuilder =
    ...
    @Override
    public long ramBytesUsed() {
      // TODO: add size for each tensor packedValue?
      return SHALLOW_SIZE
