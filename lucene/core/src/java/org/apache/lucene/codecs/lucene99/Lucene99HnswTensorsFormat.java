 ...
 * files, and requires a {@link Lucene99FlatTensorsFormat} to store the actual tensors. Each node in
 * the graph is a tensor ordinal, which maps to a single document. The tensor itself comprises
 * one or more vector values.
 *
 * <h2>.vext (tensor index)</h2>
 *
 ...
 * </ul>
 *
 * <h2>.vemt (tensor metadata) file</h2>
 *
 * <p>For each field:
 *
 * <ul>
 ...
 *   <li><b>[vlong]</b> offset to this field's index in the .vext file
...
public final class Lucene99HnswTensorsFormat extends KnnTensorsFormat {
  ...
  static final String META_CODEC_NAME = "Lucene99HnswTensorsFormatMeta";
  static final String TENSOR_INDEX_CODEC_NAME = "Lucene99HnswTensorsFormatIndex";
  static final String META_EXTENSION = "vemt";
  static final String TENSOR_INDEX_EXTENSION = "vext";
   ...
   * {@link Lucene99HnswTensorsFormat#DEFAULT_MAX_CONN}. See {@link HnswGraph} for more details.
   */
  private final int maxConn;

  /**
   * The number of candidate neighbors to track while searching the graph for each newly inserted
   * node. Defaults to {@link Lucene99HnswTensorsFormat#DEFAULT_BEAM_WIDTH}. See {@link HnswGraph}
   * for details.
   */
  private final int beamWidth;

  /** The format for storing, reading, merging tensors on disk */
  private static final FlatTensorsFormat flatTensorsFormat =
  ...
  public KnnVectorsWriter fieldsWriter(SegmentWriteState state) throws IOException {
    return new Lucene99HnswTensorsWriter(
        state,
        maxConn,
        beamWidth,
        flatTensorsFormat.fieldsWriter(state),
        numMergeWorkers,
        mergeExec);
  }
  ...
  public KnnVectorsReader fieldsReader(SegmentReadState state) throws IOException {
    return new Lucene99HnswTensorsReader(state, flatTensorsFormat.fieldsReader(state));
  }
