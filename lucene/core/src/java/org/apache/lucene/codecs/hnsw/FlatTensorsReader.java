...
/**
 * Reads tensors from an index. When searching this reader, it iterates every tensor in the index
 * and scores them
 *
 * <p>This class is useful when:
 *
 * <ul>
 *   <li>the number of tensors is small
 *   <li>when used along side some additional indexing structure that can be used to better search
 *       the tensors (like HNSW).
 * </ul>
 *
 * @lucene.experimental
 */
public abstract class FlatTensorsReader extends FlatVectorsReader {

  /** Scorer for flat tensors */
  protected final FlatTensorsScorer tensorScorer;

  /** Sole constructor */
  protected FlatTensorsReader(FlatTensorsScorer tensorScorer) {
    super(null);
